...
class StreamhubProvider : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse? {
        val videoId = Regex("Streamhub.com/video/([a-zA-Z0-9]+)").find(url)?.groups?.get(1)?.value
    ...
    private fun VideoItem.toSearchResponse(provider: StreamhubProvider): SearchResponse {
        return provider.newMovieSearchResponse(
            this.title,
            "https://www.Streamhub.com/video/${this.id}",
            TvType.Movie
        ) {
    ...
    override suspend fun loadLinks(
        ...
        loadExtractor(
            "https://www.Streamhub.com/embed/video/$data",
            subtitleCallback,
            callback
        )
        return true
